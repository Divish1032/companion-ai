...
for required in DEPLOY_DOMAIN LIVEKIT_NODE_IP LIVEKIT_API_KEY LIVEKIT_API_SECRET; do
  if [[ -z ${!required:-} ]]; then
...
envsubst '${DEPLOY_DOMAIN} ${LIVEKIT_NODE_IP} ${LIVEKIT_API_KEY} ${LIVEKIT_API_SECRET}' \
